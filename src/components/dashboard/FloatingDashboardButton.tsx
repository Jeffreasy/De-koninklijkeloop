...
export default function FloatingDashboardButton() {
    ...
    // DEBUG: Force show to check state
    const debugRole = user?.role || "null";
    const debugAuth = user ? "USER_SET" : "USER_NULL";
    ...
    return (
        ...
        >
            <div className={`relative shrink-0 ${!user ? 'grayscale' : ''}`}>
            ...
            <div className="flex flex-col text-left">
                ...
                <span className="text-xs md:text-sm font-bold text-zinc-800 dark:text-white group-hover:text-brand-orange transition-colors">
                    {user ? "Mijn Dashboard" : "Niet Ingelogd"}
                </span>
            </div>
        </a>
    );
